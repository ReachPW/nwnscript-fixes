...
// TRUE if the given creature has the given condition set
int GetAnimationCondition(int nCondition, object oCreature = OBJECT_SELF) { return (GetLocalInt(oCreature, sAnimCondVarname) & nCondition); }
...
int GetIsBusyWithAnimation(object oCreature)
{
	int bReturn = GetAnimationCondition(NW_ANIM_FLAG_IS_TALKING, oCreature) || GetAnimationCondition(NW_ANIM_FLAG_IS_INTERACTING, oCreature) || GetCurrentAction(oCreature) != ACTION_INVALID;
...
object GetRandomFriend(float fMaxDistance)
{
	object oFriend = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_FRIEND, OBJECT_SELF, d2(), CREATURE_TYPE_PERCEPTION, PERCEPTION_SEEN);

	if (GetIsObjectValid(oFriend) &&
		!GetIsPC(oFriend)
		//&& !GetIsBusyWithAnimation(oFriend) BK Feb 2003: There's not enough talking happening
		&& GetAnimationCondition(NW_ANIM_FLAG_IS_ACTIVE, oFriend) && !IsInConversation(oFriend) && !GetIsInCombat(oFriend) && GetDistanceToObject(oFriend) <= fMaxDistance)
	{
		return oFriend;
	}
...
object GetRandomStop(float fMaxDistance)
{
	...
	if (GetIsObjectValid(oStop) && GetDistanceToObject(oStop) <= fMaxDistance) return oStop;
	return OBJECT_INVALID;
}
...
// Get the caller's current friend, if set; OBJECT_INVALID otherwise
object GetCurrentFriend() { return GetLocalObject(OBJECT_SELF, "NW_ANIM_FRIEND"); }
...
// Get the caller's current interaction target, if set; OBJECT_INVALID otherwise
object GetCurrentInteractionTarget() { return GetLocalObject(OBJECT_SELF, "NW_ANIM_TARGET"); }
...
void PlayMobileAmbientAnimations_NonAvian()
{
	...
	if (!GetAnimationCondition(NW_ANIM_FLAG_IS_ACTIVE)) return;
...
void AnimActionPlayRandomImmobile()
{
	...
	if (nRoll <= 9)
	{
		if (AnimActionFindFriend(DISTANCE_LARGE)) return;
	}

	if (nRoll > 9)
	{
		// Try and interact with a nearby placeable
		if (AnimActionFindPlaceable(DISTANCE_SHORT)) return;
	}
...
void AnimActionPlayRandomCloseRange()
{
	...
	if (nRoll == 2)
	{
		if (AnimActionFindPlaceable(DISTANCE_LARGE)) return;
	...
	if (nRoll == 3)
	{
		if (AnimActionSitInChair(DISTANCE_LARGE)) return;
	}
...
void AnimActionPlayRandomMobile()
{
	...
	if (nRoll == 0)
	{
		...
		if (AnimActionGoOutside()) return;
	}

	if (nRoll == 1)
	{
		// Possibly go into an interior area
		if (AnimActionGoInside()) return;
	}
	...
	if (nRoll < 5)
	{
		...
		if (AnimActionGoToStop(1000.0)) return;
...
void AnimActionStartInteracting(object oPlaceable)
{
	...
	if (GetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE) || GetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE_CLOSE_RANGE))
	{
...
void AnimActionStartTalking(object oFriend, int nHDiff = 0)
{
	...
	if (GetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE, oFriend) || GetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE_CLOSE_RANGE, oFriend))
	{
...
int AnimActionGoHome()
{
	...
	if (GetIsObjectValid(oHome) && !GetIsDay() && GetArea(OBJECT_SELF) != GetArea(oHome))
	{
		...
		AnimActionGoOutside();
		AnimActionGoThroughDoor(GetLocalObject(OBJECT_SELF, "NW_ANIM_DOOR_HOME"));
		return TRUE;
	}
	return FALSE;
}
...
void AnimActionPlayRandomInteractAnimation(object oPlaceable)
{
	...
	if (fZDiff > 0.0)
	{
		...
		ActionPlayAnimation(ANIMATION_LOOPING_GET_LOW, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
	}
	else
	{
	...
	if (GetHasInventory(oPlaceable) || GetWillSavingThrow(oPlaceable) == 1)
	{
		...
		{
			AssignCommand(oPlaceable, DelayCommand(ANIM_LOOPING_LENGTH, ActionPlayAnimation(ANIMATION_PLACEABLE_CLOSE)));
		}
		else
		{
			AssignCommand(oPlaceable, DelayCommand(ANIM_LOOPING_LENGTH, ActionPlayAnimation(ANIMATION_PLACEABLE_OPEN)));
		}
	}
	else
	{
...
void AnimActionPlayRandomTalkAnimation(int nHDiff)
{
	...
	switch (nRoll)
	{
		...
		case 1:
			ActionPlayAnimation(ANIMATION_LOOPING_LISTEN, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
			break;
		case 2:
		case 3:
			ActionPlayAnimation(ANIMATION_LOOPING_TALK_NORMAL, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
			break;
		case 4:
			if (nHDiff < 0)
				ActionPlayAnimation(ANIMATION_LOOPING_TALK_PLEADING, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
			else
			{
				if (Random(2) == 0 && GetAnimationCondition(NW_ANIM_FLAG_CHATTER))
				{
		...
		case 5:
			...
			//  else
			ActionPlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
			break;
		case 6:
			if (GetAnimationCondition(NW_ANIM_FLAG_CHATTER))
			{
...
void AnimActionPlayRandomAnimation()
{
	...
	if (bInTavern)
	{
		...
		switch (nRoll)
		{
			...
			case 12:
			case 13:
				ActionPlayAnimation(ANIMATION_LOOPING_PAUSE_TIRED, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
				break;
			case 14:
	...
	else if (bNearAltar)
	{
		...
		switch (nRoll)
		{
			...
			case 2:
			case 3:
				ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH * 2);
				break;
			case 4:
			case 5:
				ActionPlayAnimation(ANIMATION_LOOPING_WORSHIP, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH * 2);
				break;
			case 6:
				ActionPlayAnimation(ANIMATION_LOOPING_LISTEN, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
				break;
			case 7:
				ActionPlayAnimation(ANIMATION_LOOPING_PAUSE, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
				break;
			case 8:
				ActionPlayAnimation(ANIMATION_LOOPING_PAUSE2, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
				break;
			case 9:
				AnimActionLookAround();
				break;
	...
	else
	{
		...
		switch (nRoll)
		{
			...
			case 5:
				...
				break;
			case 6:
				ActionPlayAnimation(ANIMATION_LOOPING_LOOK_FAR, ANIM_LOOPING_SPEED, ANIM_LOOPING_LENGTH);
				break;
			case 7:
				AnimActionLookAround();
				break;
