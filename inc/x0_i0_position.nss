...
void SpeakLocation(location lLoc) { SpeakString(LocationToString(lLoc)); }

// Print location --- private function for debugging
void PrintLocation(location lLoc) { PrintString(LocationToString(lLoc)); }

// Turn a location into a string. Useful for debugging.
string LocationToString(location loc) { return "(" + GetTag(GetAreaFromLocation(loc)) + ")" + " " + VectorToString(GetPositionFromLocation(loc)) + " (" + FloatToString(GetFacingFromLocation(loc)) + ")"; }


// Turn a vector into a string. Useful for debugging.
string VectorToString(vector vec) { return "(" + FloatToString(vec.x) + ", " + FloatToString(vec.y) + ", " + FloatToString(vec.z) + ")"; }
...
// This returns the change in X coordinate that should be made to
// cause an object to be fDistance away at fAngle.
float GetChangeInX(float fDistance, float fAngle) { return fDistance * cos(fAngle); }

// This returns the change in Y coordinate that should be made to
// cause an object to be fDistance away at fAngle.
float GetChangeInY(float fDistance, float fAngle) { return fDistance * sin(fAngle); }
...
vector GetChangedPosition(vector vOriginal, float fDistance, float fAngle)
{
	...
	if (vChanged.x < 0.0) vChanged.x = -vChanged.x;
	vChanged.y = vOriginal.y + GetChangeInY(fDistance, fAngle);
	if (vChanged.y < 0.0) vChanged.y = -vChanged.y;
...
// would use to set something facing exactly opposite the way of
// something else that's facing in direction fDirection).
float GetOppositeDirection(float fDirection) { return GetNormalizedDirection(fDirection + 180.0); }


// This returns the direction directly to the right. (IE, what
// you would use to make an object turn to the right.)
float GetRightDirection(float fDirection) { return GetNormalizedDirection(fDirection - 90.0); }

// This returns a direction that's a half-turn to the right
float GetHalfRightDirection(float fDirection) { return GetNormalizedDirection(fDirection - 45.0); }

// This returns a direction one and a half turns to the right
float GetFarRightDirection(float fDirection) { return GetNormalizedDirection(fDirection - 135.0); }

// This returns a direction a specified angle to the right
float GetCustomRightDirection(float fDirection, float fAngle) { return GetNormalizedDirection(fDirection - fAngle); }

// This returns the direction directly to the left. (IE, what
// you would use to make an object turn to the left.)
float GetLeftDirection(float fDirection) { return GetNormalizedDirection(fDirection + 90.0); }

// This returns a direction that's a half-turn to the left
float GetHalfLeftDirection(float fDirection) { return GetNormalizedDirection(fDirection + 45.0); }

// This returns a direction one and a half turns to the left
float GetFarLeftDirection(float fDirection) { return GetNormalizedDirection(fDirection + 135.0); }

// This returns a direction a specified angle to the left
float GetCustomLeftDirection(float fDirection, float fAngle) { return GetNormalizedDirection(fDirection + fAngle); }
...
void TurnToFaceObject(object oObjectToFace, object oTarget = OBJECT_SELF) { AssignCommand(oTarget, SetFacingPoint(GetPosition(oObjectToFace))); }
...
location GetFlankingRightLocation(object oTarget)
{
	...
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fAngleToRightFlank, fDir);
}
...
location GetFlankingLeftLocation(object oTarget)
{
	...
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fAngleToLeftFlank, fDir);
}
...
location GetOppositeLocation(object oTarget)
{
	...
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fDir, fAngleOpposite);
}
...
{
	float fDir = GetFacing(oTarget);
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fDir, fDir);
}
...
location GetBehindLocation(object oTarget)
{
	float fDir			 = GetFacing(oTarget);
	float fAngleOpposite = GetOppositeDirection(fDir);
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fAngleOpposite, fDir);
}
...
location GetAheadRightLocation(object oTarget)
{
	...
	return GenerateNewLocation(oTarget, DISTANCE_MEDIUM, fAngle, fFaceAngle);
}
...
location GetStepLeftLocation(object oTarget)
{
	...
	return GenerateNewLocation(oTarget, DISTANCE_TINY, fAngle, fDir);
}
...
location GetRandomLocation(object oArea, object oSource = OBJECT_INVALID, float fDist = 0.0)
{
	...
	if (fDist == 0.0)
	{
		...
		{
			case 0:
				fDist = DISTANCE_MEDIUM;
				break;
			case 1:
				fDist = DISTANCE_LARGE;
				break;
			case 2:
				fDist = DISTANCE_HUGE;
				break;
