...
location WMGetAmbushSpot(object oPC)
{
	...
	if (bUseDoors)
	{
		...
		{
			if (GetDistanceBetween(oDoor, oPC) > GetDistanceBetween(oDoor2, oPC) || GetLocked(oDoor) || GetPlotFlag(oDoor)) oDoor = oDoor2;
		}
...
//------------------------------------------------------------------------------
int WMGetAreaHasTable(object oArea) { return (GetLocalInt(oArea, "X2_WM_AREA_TABLE")); }

//------------------------------------------------------------------------------
// Returns TRUE if oArea has the Wandering Monster System disabled
//------------------------------------------------------------------------------
int WMGetWanderingMonstersDisabled(object oArea) { return (GetLocalInt(oArea, "X2_WM_DISABLED")); }

//------------------------------------------------------------------------------
// Sets if oArea has the Wandering Monster System disabled
//------------------------------------------------------------------------------
void WMSetWanderingMonstersDisabled(object oArea, int bDisabled = FALSE) { SetLocalInt(oArea, "X2_WM_DISABLED", bDisabled); }
...
// the system.
//------------------------------------------------------------------------------
int WMGetUseAppearAnimation(object oArea) { return GetLocalInt(oArea, "X2_L_WM_USE_APPEAR_ANIMATIONS"); }
